...
- Engine config keys: n_ctx (context window, default 4096), n_gpu_layers (0 = CPU only), lib_dir (explicit DLL folder), chat_format (named prompt format). llava_shared.dll is loaded only if present (needed for multimodal models only).
