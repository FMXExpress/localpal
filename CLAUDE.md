...
- Built-in model catalog is tiered via the TCatalogItem.Tier field (mtLowEnd/mtHighEnd): low-end (Gemma 2, Qwen 2.5, Llama 3.2, Phi 3.5, Gemmasutra, SmolLM2, SmolVLM2) runs on CPU/small GPUs and is shown by default; high-end (Qwen 3.6 27B & 35B-A3B, Gemma 4 26B-A4B & 31B) needs 24GB+ and is shown only with `model catalog --all`. Each item carries a MinRamGB hint. All are single-file Q4 GGUFs so the existing one-file downloader handles them. Selectable via ID or alphanumeric key.
